...
class SongGenerator:
    def __init__(self):
        ...
        self.genres = set(["Electronica", "Techno", "Ambient", "Synthwave", "Cyberpunk"])
    ...
    def load_data(self):
        ...
        try:
            if Path(SAVE_PATH).exists():
                ...
                self.genres = set(data.get("genres", ["Electronica", "Techno", "Ambient", "Synthwave", "Cyberpunk"]))
    ...
    def generate_random_song(self) -> Song:
        ...
        # 20% chance to create a new genre variation
        if random.random() < 0.2 and len(self.genres) >= 2:
            # Create a genre variation
            base_genres = random.sample(list(self.genres), 2)
            new_genre = self._create_genre_variation(base_genres)
            genre = new_genre
        else:
            # Use existing genre
            genre = random.choice(list(self.genres))
        
        # Generate title
        title_components = [
            ["Neon", "Digital", "Cyber", "Virtual", "Electric", "Quantum", "Cosmic", "Neural", "Binary", "Synthetic"],
            ["Pulse", "Wave", "Echo", "Dream", "Beat", "Rush", "Flow", "Drift", "Sync", "Grid", "Node", "Link", "Loop"],
            ["Protocol", "Signal", "Dimension", "Reality", "System", "Network", "Matrix", "Circuit", "Code", "Light"]
        ]
        
        # 50% chance to use a two-word title, 50% chance for three words
        if random.random() < 0.5:
            title = f"{random.choice(title_components[0])} {random.choice(title_components[1])}"
        else:
            title = f"{random.choice(title_components[0])} {random.choice(title_components[1])} {random.choice(title_components[2])}"
        ...
        if len(available_tags) < num_tags:
            ...
            new_tags_needed = num_tags - len(available_tags)
            ...
            for _ in range(new_tags_needed):
    ...
    def _create_genre_variation(self, base_genres: List[str]) -> str:
        """Create a new genre variation based on existing genres"""
        # Combine two genres or add a modifier
        modifiers = ["Future", "Neo", "Post", "Hyper", "Deep", "Vapor", "Glitch", "Dark", "Lo-fi", "Hi-tech"]
        fusion_types = ["wave", "step", "core", "punk", "tronic", "beat", "synth", "hop", "tech", "funk"]
        
        # 50% chance for a fusion, 50% chance for a modifier
        if random.random() < 0.5:
            # Create a fusion
            base1 = base_genres[0]
            base2 = base_genres[1]
            # Extract root words (remove common suffixes)
            for suffix in ["wave", "step", "core", "punk", "tronic", "beat", "synth", "hop", "tech", "funk"]:
                base1 = base1.replace(suffix, "")
                base2 = base2.replace(suffix, "")
            # Create fusion
            new_genre = f"{base1}{random.choice(fusion_types)}"
        else:
            # Add a modifier
            modifier = random.choice(modifiers)
            base = random.choice(base_genres)
            new_genre = f"{modifier} {base}"
        
        # Add to our genres set
        ...
        if self.auto_generation_active:
            try:
                while not self.auto_queue.empty():
                    ...
                    # Note: We don't show success messages here because they would
                    # clutter the UI during auto-generation
    ...
    def generate_song_description(self) -> str:
        """Generate a random but coherent song description with many possible combinations"""
        ...
        mood_descriptors = [
            ...
            "A distorted", "A dynamic", "An ethereal", "A raw", "A polished"
        ]
        
        style_descriptors = [
            "futuristic", "retro-inspired", "experimental", "minimalist", "complex", 
            "bass-heavy", "melodic", "dark", "upbeat", "industrial",
            "synthwave", "ambient", "dystopian", "cyber-noir", "glitched",
            "cinematic", "urban", "organic", "surreal", "technological"
        ]
        
        sound_elements = [
            "synth sequences", "drum patterns", "bass lines", "vocal samples", "arpeggios",
            "pads", "field recordings", "digital artifacts", "rhythmic glitches", "reverberations",
            "noise textures", "harmonic layers", "processed beats", "drone elements", "percussion loops",
            "modular sounds", "analog warmth", "digital precision", "sonic manipulations", "evolving textures"
        ]
        ...
        emotional_impacts = [
            ...
            "balancing between tension and resolution",
            ...
            "challenging expectations with surprising turns",
            "offering a moment of peaceful reflection",
            "creating a soundtrack for urban exploration",
            "capturing the essence of digital-age anxiety",
            "translating visual aesthetics into sound",
            "celebrating the fusion of human creativity and technology",
            "leaving space for the listener's own imagination"
        ]
        
        # Generate a description using random combinations of the components
        description = (
            f"{random.choice(mood_descriptors)} {random.choice(style_descriptors)} track that blends {random.choice(sound_elements)} "
            f"with {random.choice(sound_elements)}. The composition {random.choice(musical_journeys)}, "
            f"{random.choice(emotional_impacts)}."
        )
...
with col1:
    ...
    with st.form("song_creation_form"):
        ...
        genre_options = list(song_generator.genres)
        ...
        if submitted:
            ...
            if generate_desc:
                description = song_generator.generate_song_description()
    ...
    for achievement_key, achievement in song_generator.achievements.items():
        ...
        if current_level < max_level:
            ...
            if achievement_key == "songs_created":
                ...
                current_value = len(song_generator.genres)
...
with stats_col2:
    st.metric("Unique Genres", len(song_generator.genres))
